...
| **硬 8-bit JND 階數最佳化** | 在可選的保留階數預算下重新分配目前路徑的 256 個裝置碼，套用前可比較原始／最佳化 transfer 與相鄰 JND 曲線 |
...
| **基本 / 進階 / 階數 / 診斷分頁** | 切換階調控制、影像細節、硬 8-bit JND 重新分配與檢查工具 |
| **最佳階數** | 讓保留階數預算符合目前 GSDF/CSDF 路徑、套用受限轉換表並開啟階數分頁，不重設其他設定 |
| **語言選單** | 不需重新建置即可切換介面語言 |
| **主題按鈕** | 切換深色與淺色面板 |
