...
test('panel keeps project-owned GSDF pattern and chart logic', () => {
  ...
  assert.match(panelSource, /REFERENCE_ORDERED_DITHER_MATRIX/);
  ...
  assert.match(bidirectionalBlock, /applyActiveReferenceToneTable\(/);
  ...
  assert.match(bidirectionalBlock, /applyReferenceOrderedDither\(context, renderWidth, renderHeight, settings\)/);
  ...
  assert.match(bidirectionalBlock, /drawCmyRgbContinuousGradientBand\(context, 0, 0, renderWidth, renderHeight, transferTable, settings\)/);
