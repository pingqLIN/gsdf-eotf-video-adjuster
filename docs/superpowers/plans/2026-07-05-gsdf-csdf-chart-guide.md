# GSDF/CSDF Chart Guide Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a Traditional Chinese GSDF/CSDF chart guide with visual aids that explain chart regions, extension controls, and which drawing surfaces follow the active adjustment.

**Architecture:** Keep this as a documentation-only change. Reuse existing README screenshots for the curve and extension panel, then add one project-owned SVG diagram under `docs/assets/` for the affected-vs-fixed drawing-surface split.

**Tech Stack:** Markdown, SVG, existing PNG documentation assets, PowerShell validation, Git.
...
- Create: `docs/assets/gsdf-csdf-affected-surfaces.svg`
  - Responsibility: Static explanatory diagram that separates surfaces affected by current GSDF/CSDF settings from fixed reference surfaces.
- Create: `docs/gsdf-csdf-chart-guide.zh-tw.md`
  - Responsibility: User-facing Traditional Chinese guide with implementation-boundary notes and visual callouts.
- Reuse: `assets/readme-curve-chart.png`
  - Responsibility: Existing curve-chart screenshot for the chart-elements explanation.
- Reuse: `assets/readme-panel-expanded-workspace.png`
  - Responsibility: Existing extension workspace screenshot for control-surface explanation.

Do not modify `src/types.ts`, `src/components/GSDFChart.tsx`, `extension/content.js`, build scripts, or README indexes in this plan.

---

### Task 1: Add Affected-Surfaces Diagram

**Files:**
- Create: `docs/assets/gsdf-csdf-affected-surfaces.svg`

- [ ] **Step 1: Create the SVG diagram**
...
Expected: commit succeeds with one new SVG file.

---

### Task 2: Add Traditional Chinese Guide
...
Use `apply_patch` to add this exact file. The image paths are relative to `docs/`.
...
- [ ] **Step 2: Verify reused image paths exist**
...
Expected: commit succeeds with one new Markdown guide.

---

### Task 3: Validate Documentation Package
...
- [ ] **Step 2: Scan for unfinished markers and overclaims**
...
Expected: no output. If a negated disclaimer triggers the scan, reword it so the command only catches actual overclaims.

- [ ] **Step 3: Validate Markdown image references**
...
- [ ] **Step 4: Commit validation touch-ups if needed**

If any validation step required wording changes, run:
...
Expected: commit succeeds only if validation caused a file change. If validation passes without changes, do not create an empty commit.
...
Expected: branch is ahead of `origin/main` by the new documentation commits, with no unstaged or untracked files except ignored `.clean/` contents.
